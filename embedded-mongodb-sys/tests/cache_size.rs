...
use embedded_mongodb_sys::{CacheSize, Client, EngineOptions, Session};
...
fn the_cache_size_option_is_the_cache_wiredtiger_reports_running_with() {
    ...
    let session = client.open_session().expect("opening a session");

    let status = run(&session, doc! { "serverStatus": 1, "wiredTiger": 1 });
    let reported = status
...
fn run(session: &Session, command: Document) -> Document {
