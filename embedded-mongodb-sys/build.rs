...
///
/// Bazel is the only command here that needs this. The others -- curl and wget in `fetch`,
/// git in `check_not_stale`, the compiler probes, and install_name_tool and codesign in
/// `run` -- run to completion and leave nothing behind, so they can go on writing straight
/// to cargo. Anything added later that forks a daemon, a compiler wrapper like sccache
/// included, belongs on this path instead.
fn run_logged(command: &mut Command, log_path: &Path) -> std::io::Result<ExitStatus> {
