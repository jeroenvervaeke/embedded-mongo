...
//! dedicated worker threads, one per session, that do that occupying on the caller's
...
// not among them -- it is this crate's own pool policy, defined in `options`.
...
pub use options::{Concurrency, OpenOptions};
